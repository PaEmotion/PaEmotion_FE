...
class _BudgetEditScreenState extends State<BudgetEditScreen> {
  ...
  Set<String> _deletedCategories = {}; // 삭제된 항목 추적
  ...
  Future<void> _loadBudgets() async {
    final savedBudgets = await BudgetStorage.loadBudgets(_currentMonth);
    setState(() {
  ...
  void _addBudgetItem() {
    final selectedCategories = budgetItems.map((e) => e['category'] as String).toSet();
    final availableCategories = allCategories.where((c) => !selectedCategories.contains(c)).toList();
  ...
  void _removeBudgetItem(int index) {
    final removedCategory = budgetItems[index]['category'] as String;
    setState(() {
      _deletedCategories.add(removedCategory);
  ...
  Future<void> _saveBudgets() async {
    ...
    for (final item in budgetItems) {
      ...
      final budget = Budget(month: _currentMonth, category: category, amount: amount);
      await BudgetStorage.saveBudget(budget);
    }

    for (final category in _deletedCategories) {
      await BudgetStorage.deleteBudget(_currentMonth, category);
    }
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              if (message.contains('수정되었습니다') || message.contains('삭제되었습니다')) {
          ...
          children: [
                            ...
                            children: [
                              Expanded(
                                ...
                                child: DropdownButton<String>(
                                  ...
                                  isExpanded: true,
                                  items: allCategories
                                      .where((c) => c == category || !selectedCategories.contains(c))
                                  ...
                                  onChanged: (newCategory) {
                                    ...
                                    if (selectedCategories.contains(newCategory) && newCategory != category) {
                                      ScaffoldMessenger.of(context).showSnackBar(
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('수정 저장하기', style: TextStyle(fontSize: 16)),
              ),
            ),
              ...
              child: ElevatedButton(
                onPressed: () {
                  showDialog(
                    context: context,
                    builder: (_) => AlertDialog(
                      title: const Text('예산 삭제 확인'),
                      content: const Text('정말 예산을 모두 삭제하시겠습니까?'),
                      actions: [
                        TextButton(
                ...
                child: const Text('예산 모두 삭제하기', style: TextStyle(fontSize: 16)),
              ),
            ),
          ],
        ),
